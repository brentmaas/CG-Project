...
GLuint loadComputeShader3(const char* file, GLuint type){
...
GLuint generateProgram3(const char* computeFile){
	GLuint computeShaderID = loadComputeShader3(computeFile, GL_COMPUTE_SHADER);
...
Simulation2::Simulation2(int N, float g, float hr, float hz, int seed){
	...
	for(int i = 0;i < N;i++){
		...
		v.x = 0;// vproj * (x.z - mmp.z) / rproj;
		v.y = 0;//vtot * sqrt(std::max(0.0f, 1 - costheta * costheta));
		v.z = 0;//-vproj * (x.x - mmp.x) / rproj;
		v.w = 0;
	}
	...
	computeProgram = generateProgram3("resources/shaders/particles_e_conserv.compute");
